        ...
        for epoch in range(I_num_epoch):
            ...
            for i, (batch_data, label) in enumerate(train_loader):
                ...
                real_freq = gan_nerual.compute_spectrum(real,
                                                        ...
                                                        train_step=S_I_step,
                                                        path=config["save_plot_tb_path"] + config["training_inference_path"])
                ...
                #grad loss
                grad_loss=criterion_grad(fake_condition[:,100:,:],fake_condition[:,100:,:])
                grad_loss_list.append(grad_loss)
                #ini loss
                ini_loss=criterion_ini(fake_condition[:,0,:],fake_condition[:,0,:])
                ini_loss_list.append(ini_loss)
                #fourier loss
                fourier_loss=criterion_fourier(generator_freq,real_freq)
                fourier_loss_list.append(fourier_loss)

                infer_loss=mse_loss+config["lamba_grad"]*grad_loss+config["lamba_ini"]*ini_loss+\
                            config["lamba_fourier"]*fourier_loss
            ...
            writer.add_scalars("loss", {"inference_loss": epoch_inference_loss,
                                        "fourier_loss": epoch_fourier_loss,
                                        "ini_loss": epoch_ini_loss,
                                        "grad_loss": epoch_grad_loss,
            ...
            with torch.no_grad():
                pass
               # eval_inference_model(eval_data=valid_loader, model_I=S_I,model_O=S_Omega, data_t=data_t, eval_epoch=epoch)
        ...
        for epoch in range(Omgea_num_epoch):
            ...
            for i,(batch_data,label) in enumerate(train_loader):
               ...
               real_freq = gan_nerual.compute_spectrum(real,
                                                       ...
                                                       path=config["save_plot_tb_path"]+config["training_omega_data_path"])
...
def eval_inference_model(eval_data,model_I,model_O,data_t,eval_epoch):
    ...
    for i, (batch_data, label) in enumerate(eval_data):
        ...
        real_freq= gan_nerual.compute_spectrum(real,
                                                ...
                                                path=config["save_plot_tb_path"] + config["eval_data_path"]+config["eval_inference_path"]
                                                +"\real")
    ...
    writer.add_scalars('eval_loss', {       'eval_freq_loss': eval_freq_loss,
...
def eval_omega_model(eval_data,model,data_t,eval_epoch):
    ...
    for i, (batch_data, label) in enumerate(eval_data):
        ...
        real_freq = gan_nerual.compute_spectrum(real,
                                                ...
                                                path=config["save_plot_tb_path"] + config["eval_data_path"]+config["eval_omega_path"])
        ...
        #cost_matirx infinite_norm
        cost_matrix=uf.cost_matrix() #[batch,50,50，2]
        infinite_norm= [uf.infinity_norm(cost_matrix[i]) for i in range(cost_matrix.shape[0])]
        infinite_norm_avr=np.mean(infinite_norm)
        eval_infinite_cost_list.append(infinite_norm_avr)
    ...
    eval_infinite_cost=sum(eval_infinite_cost_list)/len(eval_infinite_cost_list)
    writer.add_scalars("eval",{"mse_loss":eval_mse_loss,
                                "u_stat":eval_u,
                                "cost_norm":eval_infinite_cost},eval_epoch)
