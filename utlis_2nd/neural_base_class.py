...
class Omgea_MLPwith_residual_dict(nn.Module):
    ...
    def __init__(self,
                 ...
                 device_type="cuda",
                 sample_info=[None,1],#["gumble/topk","prob_number=1,2"] none means pick all
        ...
        elif Combination_basis is "GPT":
            self.basis_function = ["gpt_function","sin", "cos"]
        self.device_type=device_type

        # 解构赋值来提取列表中的信息
        self.sample_type, self.prob_sample_numb = sample_info
        self.soft_learnable, self.soft_temp_value = soft_arg_temp_info

        # 使用布尔表达式直接赋值
        self.sample_en = self.sample_type in ("Topk")

        if(self.soft_learnable == True):
            #can upgrade
    ...
    def Return_sample_freq_index(self,
        ...
        # 根据 self.gumble_en 的值决定使用 Gumbel softmax 还是直接赋值
        process_tensor =  freq_distrubtion_tensor

        # 根据 self.sample_type 的值决定采样方法
        if self.sample_type == "Topk":
            value, sample_index_vari = torch.topk(
                process_tensor[:, 1:, :],
                dim=1,
                k=self.prob_sample_numb
            )
        elif self.sample_type == "All":
            soft_prob = F.softmax(process_tensor / self.softarg_temp, dim=1).requires_grad_(True)
            indices = self.buffer_freq_index.view(1, -1, 1).expand_as(process_tensor)
            sample_index_vari = (soft_prob * indices).sum(dim=1).unsqueeze(1) #加权索引
    ...
    def parse_and_create_function(self,expr_str):
        # 解析表达式
        ...
        def torch_factorial(x):
            return torch.from_numpy(sp_factorial(x.numpy(), exact=False))

        # 将 sympy 表达式转换为 PyTorch 表达式
        pytorch_expr = sympy.lambdify(symbols[0], expr,
                                      modules={"sin": torch.sin, "cos": torch.cos, "exp": torch.exp, "log": torch.log,
                                               "factorial": torch_factorial})

        def function(x):

            return pytorch_expr(x)

        return function
    def gpt_function(self,data_t,function="expr_str",batch_num=256):
        #用pytorch 来实现function data_t=[1,100,1]
        #function is a string
        #return [256,100,1]
        # 示例使用
        expr_str = function
        f = self.parse_and_create_function(expr_str)

        # 使用 PyTorch 张量计算函数值
        x_value = torch.tensor(data_t)  # 示例输入
        result = f(x_value)#[1,100,1]
        result= result.repeat(batch_num,1,1)
        result=result.reshape(batch_num,100)
        print(result)
        return result
    ...
    def return_pred_data(self,coeff_tensor,
        ...
        prior_knowledge_matrix=self.basis_function
        ...
        elif self.sample_en==False:#全部都需要使用 频率
        ...
        for i, exprs in enumerate(prior_knowledge_matrix):
            ...
            elif exprs =="gpt_function":
                z1_left_matirx[:, :, 0] = self.gpt_function(data_t,function="x**4",batch_num=batch_num)  #return [batch,100,1]
                z2_left_matirx[:, :, 0] = self.gpt_function(data_t,function="x**4",batch_num=batch_num)
        ...
        Sample_omega_index_pick=Sample_omega_index_pick.repeat(1,1,prior_omega_knowledge_number,1).to(self.device_type)
