...
import utlis_2nd.post_savefig as post
def expr(expr_config,train_type="omega_net"):
    ...
    if (train_type=="inference_net"):
        print("train_inference_net")
        train_inference_actor(
                              co_train_actor,
                              config_device=expr_config["device"])
        test_dict=test_inference_model(co_train_actor)

        for epoch in range(1,expr_config["Inference_num_epoch"],10):
            print("epoch",epoch)
            post.pre_proess_savefig(folder_path=expr_config["expr_data_path"]+"/", epoch=epoch)

        print("have post_proess")
